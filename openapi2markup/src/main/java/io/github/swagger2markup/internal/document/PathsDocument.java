...
public class PathsDocument extends MarkupComponent<PathsDocument.Parameters> {
    ...
    public PathsDocument(OpenApi2MarkupConverter.Context context) {
        super(context);
    ...
    private void buildsPathsSection(MarkupDocBuilder markupDocBuilder, Map<String, PathItem> paths) {
        ...
        if (CollectionUtils.isNotEmpty(pathOperations)) {
            if (config.getPathsGroupedBy() == GroupBy.AS_IS) {
                ...
                Validate.notEmpty(context.getOpenApi().getTags(), "Tags must not be empty, when operations are grouped by tags");
                ...
                Map<String, Tag> tagsMap = TagUtils.toSortedMap(context.getOpenApi().getTags(), config.getTagOrdering());
    ...
    private String getHostname() {
        if (config.isHostnameEnabled()) {
          if (context.getOpenApi().getServers() != null && ! context.getOpenApi().getServers().isEmpty()) {
            URL url;
            try {
              url = new URL(context.getOpenApi().getServers().get(0).getUrl());
              return StringUtils.defaultString(url.getHost());
    ...
    private String getBasePath() {
        if (config.isBasePathPrefixEnabled()) {
          if (context.getOpenApi().getServers() != null && ! context.getOpenApi().getServers().isEmpty()) {
            ...
            try {
              url = new URL(context.getOpenApi().getServers().get(0).getUrl());
              return StringUtils.defaultString(url.getPath());
            } catch (MalformedURLException e) {
              throw new IllegalArgumentException(e);
            }
          }
     ...
     */
    private void buildOperation(MarkupDocBuilder markupDocBuilder, PathOperation operation, OpenApi2MarkupConfig config) {
        if (config.isSeparatedOperationsEnabled()) {
