...
OpenApi2Markup converts a OpenAPI JSON or YAML specification into either **AsciiDoc**, **GitHub Flavored Markdown** or **Atlassian Confluence Wiki** documents. By default the OpenApi2Markup converts a OpenAPI specification into four AsciiDoc files: __overview.adoc__, __paths.adoc__ , __security.adoc__ and __definitions.adoc__. But you can also convert a OpenAPI specification into only one file or a String.


==== Conversion of a local OpenAPI file

The entry point of the OpenApi2Markup API is the ``OpenApi2MarkupConverter`` class. This class provides static factory methods to create a `OpenApi2MarkupConverter.Builder`. 

[source,java,indent=0]
----
include::../../test/java/io/github/swagger2markup/DocumentationTest.java[tags=localOpenAPISpec]
----

1.  Create a ``OpenApi2MarkupConverter.Builder`` by specifying the Path to the local file 
2.  Build an instance of the ``OpenApi2MarkupConverter``
...
[source,java,indent=0]
----
include::../../test/java/io/github/swagger2markup/DocumentationTest.java[tags=remoteOpenAPISpec]
----

1.  Create a ``OpenApi2MarkupConverter.Builder`` by specifying the URL to the remote file
2.  Build an instance of the ``OpenApi2MarkupConverter``
3.  Invoke ``toFolder`` by specifying the output directory
...
You can convert the OpenAPI specification into a file.
...
You can convert the OpenAPI specification to a String.
...
OpenApi2Markup provides several options to configure the OpenApi2MarkupConverter:
...
You can configure the OpenApi2MarkupConverter by using the `OpenApi2MarkupConfigBuilder` to build a custom `OpenApi2MarkupConfig`. The `OpenApi2MarkupConfigBuilder` can be used to define OpenApi2Markup properties with a fluent API.
...
1. Create a `OpenApi2MarkupConfigBuilder` using the default constructor.
2. Configure the output `MarkupLanguage`
3. Configure the output `Language`
4. Configure additional OpenApi2Markup properties
5. Build an instance of the `OpenApi2MarkupConfig`
6. Use the custom OpenApi2MarkupConfig

You can also create a `OpenApi2MarkupConfig` from a Properties file, a `Map` or a Apache Commons Configuration.
...
2. Create a `OpenApi2MarkupConfigBuilder` using the proper constructor.
...
1. Create a `Map` and configure the `OpenApi2MarkupProperties`.
2. Create a `OpenApi2MarkupConfigBuilder` using the proper constructor.
...
==== OpenApi2Markup properties

The properties of OpenApi2Markup are defined in the class `io.github.swagger2markup.OpenApi2MarkupProperties`.
The properties are considered in the following order:

1. Java System properties
2. Custom properties
3. Default properties (included in OpenApi2Markup)

The following tables list all available properties of OpenApi2Markup:
...
|swagger2markup.swaggerMarkupLanguage| Specifies the markup language used in OpenAPI descriptions. | ASCIIDOC, MARKDOWN, CONFLUENCE_MARKUP | MARKDOWN
...
[options="header"]
.Properties which configure the order of OpenAPI Objects
|===
|Name | Description |  Possible Values | Default
...
OpenApi2Markup uses http://www.slf4j.org/[SLF4J] for all internal logging, but leaves the underlying log implementation open. To change the log level, you have the set the log level of the `io.github.swagger2markup` package.
