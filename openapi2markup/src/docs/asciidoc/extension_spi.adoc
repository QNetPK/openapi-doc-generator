...
OpenApi2Markup provides an Extension SPI to extend the functionality of OpenApi2Markup. Five types of extension are available. An extension is an abstract class which must be implemented.

[options="header"]
.OpenApi2Markup extensions
|====
...
| <<OpenAPIModelExtension>> | io.github.swagger2markup.spi.OpenAPIModelExtension | Can be used to modify the OpenAPI model before it is converted
|====
...
OpenApi2Markup extensions must be registered in the `OpenApi2MarkupExtensionRegistry`. The default `OpenApi2MarkupExtensionRegistry` of OpenApi2Markup uses the `java.util.ServiceLoader` to load and register OpenApi2Markup extensions automatically.
...
*  When a `.jar` file containing your extension is added to the classpath of OpenApi2Markup, the extensions will be automatically registered.

==== Manual registration

To register your extension manually, you have to use the `OpenApi2MarkupExtensionRegistryBuilder`.

[source,java,indent=0]
----
include::../../test/java/io/github/swagger2markup/DocumentationTest.java[tags=swagger2MarkupExtensionRegistryBuilder]
----
1. Create a ``OpenApi2MarkupExtensionRegistryBuilder`` using the default constructor
2. Register your custom extension
3. Build an instance of `OpenApi2MarkupExtensionRegistry`
4. Use the custom OpenApi2MarkupExtensionRegistry
...
==== OpenAPIModelExtension

The OpenAPIModelExtension allows to modify the OpenAPI model before it is processed by OpenApi2Markup. For example, you could use this extension to delete Paths from the OpenAPI model which should not be rendered.

Example: 

[source,java,indent=0]
----
include::../../test/java/io/github/swagger2markup/builder/MyOpenAPIModelExtension.java[tags=MyOpenAPIModelExtension]
----
1. You can change any OpenAPI model property
2. You could even remove elements from the OpenAPI model

[[extension_commons_content_markup]]
=== Content markup language

Most content extensions supports to provide content in any markup language which will be *converted, if needed, to configured OpenApi2Markup converter markup language* at import.

The content files markup language can be any https://github.com/OpenApi2Markup/markup-document-builder[markup-document-builder] supported language : 
...
OpenApi2Markup provides some extensions which can be used out-of-the-box. You just have to add the extension to your classpath.
